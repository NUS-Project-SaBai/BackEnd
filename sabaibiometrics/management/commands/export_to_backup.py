...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # Loop over all models and export their data to the PostgreSQL backup database
        for model in sorted_models:
            try:
                self.stdout.write(f"Exporting data for model: {model.__name__}...")
                objects = model.objects.all()

                for obj in objects:
                    try:
                        # Save the object using the 'sabai_backup' database
                        obj.save(using=backup_db_name)
                        print("saved")
                    except IntegrityError as e:
                        self.stdout.write(f"IntegrityError: {e}")
                    except Exception as e:
