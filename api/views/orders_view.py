...
class OrderView(APIView):
    ...
    def get(self, request, pk=None):
        ...
        serializer = OrderSerializer(orders, many=True, context={
                                     "include_medication_review": True})
    ...
    def patch(self, request, pk):
        ...
        if order.medication_review.order_status == "APPROVED" or order.medication_review.order_status == "CANCELLED":
            return Response({f"message": "Order already is already {order.medication_review.order_status}"})
        ...
        if serializer.is_valid(raise_exception=True):
            with transaction.atomic():
                if order_status == "APPROVED":
                    MedicationView().update_quantity(quantityChange=order.medication_review.quantity_changed,
