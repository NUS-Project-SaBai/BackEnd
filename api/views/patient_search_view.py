...
from sabaibiometrics.settings import ENABLE_FACIAL_RECOGNITION, USE_DUMMY_FACIAL_RECOGNITION
...
class PatientSearchView(APIView):
    ...
    def post(self, request):
        if ENABLE_FACIAL_RECOGNITION:
        ...
        elif USE_DUMMY_FACIAL_RECOGNITION: # just select first 3 patients
            ...
            return Response("Actual and dummy facial recognition are both not enabled", 503)
        serializer = PatientSerializer(
            patients, many=True, context={"confidence": face_encoding}
        )
        response = Response(serializer.data)
        if USE_DUMMY_FACIAL_RECOGNITION:
            response.headers["x-is-dummy-data"] = "True"
        return response
